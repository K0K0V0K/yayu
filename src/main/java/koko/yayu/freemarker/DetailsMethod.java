...
public class DetailsMethod implements TemplateMethodModelEx {
    ...
    public Object exec(List arguments) {
        ...
        return json.keySet()
            .stream()
            .sorted()
            .map(key -> createTableLine(key, json.get(key)))
    ...
    private String createValueField(String key, Object value) {
        ...
        if ("elapsedTime".equals(key)) {
            return DurationFormatUtils.formatDuration(
                Long.parseLong(value.toString()), "HH:MM:SS", true);
        }
    ...
    private String printJSONObject(JSONObject value) {
        ...
        if (value.keySet().size() == 1 && value.has("entry")) {
            return value.getJSONArray("entry").toList().stream()
                .map(o -> (HashMap) o)
                .map(o -> String.format("<span class=\"tag\">%s: %s</span> ",
                    o.get("key"),
                    o.get("value"))
                )
                .sorted()
